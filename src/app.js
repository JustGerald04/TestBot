...
class TitanBot extends Client {
  ...
  startWebServer() {
    ...
    // ✅ Required for parsing JSON in POST requests
    ...
    // ✅ Counting routes registered here
    registerCountingRoutes(app);
    ...
    const startServer = (port, attempt = 0) => {
      ...
      server.on('error', (error) => {
        ...
        if (!hasStartedListening && errorCode === 'EADDRINUSE' && attempt < maxPortRetryAttempts) {
          startupLog(`Port ${port} is already in use. Trying port ${port + 1}...`);
